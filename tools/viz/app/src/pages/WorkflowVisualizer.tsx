...
export default function WorkflowVisualizer() {
  ...
  const fetchWorkflowsFromMDMS = async () => {
    ...
    try {
      ...
      const endpoint = "/egov-mdms-service/v2/_search";
      const errorMsg = `Failed to fetch workflows from ${endpoint}: ${
        err instanceof Error ? err.message : String(err)
      }`;
      setError(errorMsg);
      toast({
        title: "API Error",
        description: `MDMS Service API (${endpoint}) failed: ${
          err instanceof Error ? err.message : String(err)
        }`,
        variant: "destructive",
      });
  ...
  const loadWorkflowConfig = (config: ServiceConfiguration) => {
    try {
      ...
      const transformedWorkflow: Workflow = {
        ...
        businessService:
          config.data.workflow.businessService || config.uniqueIdentifier,
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50">
      <WorkflowHeader isLoading={isLoading} error={error} message={message} />
      ...
      <div className="flex-1 min-h-0">
        ...
        >
          ...
          <div
            className="w-1/2 flex flex-col bg-white border-r border-gray-200"
          ...
          <div
            ...
            style={{ maxWidth: "50%", overflow: "hidden", minHeight: "600px" }}
          >
            <DiagramViewerPanel
              activeTab={activeTab}
