...
export const getColumns = (
...
): ColumnDef<LocalizationString>[] => [
  {
    ...
    cell: ({ getValue }) => {
      ...
      return (
        <div className="text-sm font-medium break-all" style={{ wordWrap: 'break-word', overflowWrap: 'break-word', wordBreak: 'break-all' }}>
          {value}
        </div>
      );
    },
  },
  {
    ...
    cell: ({ getValue }) => {
      const value = getValue() as string;
      return (
        <div className="text-sm break-all" style={{ wordWrap: 'break-word', overflowWrap: 'break-word', wordBreak: 'break-all' }}>
          {value}
        </div>
      );
    },
  ...
  ...SUPPORTED_LANGUAGES.map((lang): ColumnDef<LocalizationString> => ({
    ...
    maxSize: 300,
    cell: ({ row }) => {
      const value = (row.original.translations && row.original.translations[lang]) || '';
      return (
        <EditableCell
          value={value}
          onSave={(newValue) =>
            onUpdateString(row.original.code, row.original.module, lang, newValue)
          }
        />
      );
    },
  })),
  {
    ...
    maxSize: 150,
    cell: ({ row }) => {
      const isComplete = row.original.translations?.fr && row.original.translations.fr.trim() !== '';
      return (
